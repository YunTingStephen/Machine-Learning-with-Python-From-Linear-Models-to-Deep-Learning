...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    # Your code here
    z = label * (theta.T.dot(feature_vector) + theta_0)
    if z >= 1:
        return 0
    else:
        return 1 - z
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    # Your code here
    hinge_sum = 0
    full_theta_x = theta.dot(feature_matrix.T) + theta_0  #this is 1-D array, shape is (n,)
    full_theta_x = full_theta_x.reshape((1, full_theta_x.shape[0]))  #chagne it to (1,n)
    for i in range(feature_matrix.shape[0]):
        hinge_sum += max(0, 1 - labels[i]* full_theta_x[0][i])
    

    return hinge_sum / feature_matrix.shape[0]
#pragma: coderesponse end
...
def pegasos(feature_matrix, labels, T, L):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            # Your code here
            theta, theta_0 = pegasos_single_step_update(feature_matrix[i], labels[i], L, 1/np.sqrt(count), theta, theta_0)
            count += 1 
    return (theta,theta_0)
